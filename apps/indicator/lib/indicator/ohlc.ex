defmodule Indicator.Ohlc do
  ...
  def merge_price(%__MODULE__{} = ohlc, price, trade_time) do
    if within_timeframe(ohlc.start_time, ohlc.duration, trade_time) do
  ...
  def within_timeframe(start_time, duration, trade_time) do
  ...
  def generate_ohlcs(symbol, price, trade_time) do
    ...
    |> Enum.map(&generate_ohlc(symbol, &1, price, trade_time))
  end
