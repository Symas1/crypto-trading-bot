defmodule Hedgehog.Strategy.Naive.FormulaTest do
  ...
  test "Strategy places a buy order" do
    ...
    BinanceMock
    |> stub(:order_limit_buy, fn "ABC", "50.000", "0.800000", "GTC" -> {:ok, expected_order} end)
  ...
  @tag :unit
  test "Generating skip rebuy decision because no free chunks" do
    assert Formula.generate_decision(
             %TradeEvent{
               price: "0.89"
             },
  ...
  @tag :unit
  test "Generating skip(rebuy) decision because price is not low enough" do
    assert Formula.generate_decision(
             %TradeEvent{
               price: "0.9"
             },
